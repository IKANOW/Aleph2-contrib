...
public class MockHadoopTestingService {
	...
	public void addFileToBucketStorage(final InputStream local_stream, final DataBucketBean bucket, String subservice_suffix) throws IOException {
	...
	public int numFilesInInputDirectory(final DataBucketBean bucket) {
		return numFilesInBucketStorage(bucket, IStorageService.TO_IMPORT_DATA_SUFFIX);
	}
	
	public int isFileInTransientJobOutput(final DataBucketBean bucket, final String job) {
		return numFilesInBucketStorage(bucket, IStorageService.TRANSIENT_DATA_SUFFIX + job);
	}
